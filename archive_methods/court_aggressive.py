...
def run(playwright: Playwright) -> None:
    ...
    while not element_clicked:
        page1.reload()  # Refresh the page on each iteration
        try:
            # Try to find and click the element immediately after reloading the page
            ...
            # If the element is not found, this block catches the exception
            # No need to sleep here; immediately attempt to refresh again
            print(f"Encountered an error: {e}, retrying now...")
            continue  # Immediately proceed to the next iteration to refresh again
